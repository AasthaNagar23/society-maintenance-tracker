...
app.mount(
    "/uploads",
    StaticFiles(directory=STATIC_UPLOAD_DIR),
    name="uploads"
)
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "https://society-maintenance-tracker-1-yomk.onrender.com",
    ],
...
def get_all_complaints(
    ...
    def overdue_status(complaint):
        return 1 if complaint.is_overdue else 0

    complaints.sort(
        key=lambda complaint: (
            -overdue_status(complaint),
...
def filter_complaints(
    ...
    # -----------------------------------------------------
    # OVERDUE COMPLAINTS FIRST
    # -----------------------------------------------------

    complaints.sort(
        key=lambda complaint: (
            not complaint.is_overdue,
            -complaint.created_at.timestamp()
        )
    )
...
def delete_complaint(
    ...
    # Delete associated photo file if present
...
def admin_dashboard_summary(
    ...
    for complaint in all_complaints:
        ...
        if category not in complaints_by_category:
            complaints_by_category[category] = 0

        complaints_by_category[category] += 1
...
def create_notice(
    ...
    # -----------------------------------------------------
    # SEND EMAIL TO ALL RESIDENTS FOR IMPORTANT NOTICES
    # -----------------------------------------------------

    if new_notice.is_important:
...
# =========================================================
# NOTICE BOARD - GET SINGLE NOTICE
# =========================================================
